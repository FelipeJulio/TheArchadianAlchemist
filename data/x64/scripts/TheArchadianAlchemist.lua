...
function symbols.refresh()
	...
	if addr then memory.u8[addr] = 1 end
end
	...
	for equipmentId, state in pairs(equipment.state) do
		if state.element and state.element.id then
			...
			if elementId >= 1 and elementId <= 8 then
				for id = 1, 8 do equipment.write(equipmentId, "element", id, 0) end
...
function handlers.onLoad(filepath, savedData)
	...
	if savedData and type(savedData) == "table" then
		...
		if savedData.questStatus then unlock.setQuestStatus(savedData.questStatus) end

		symbols.refresh()
	end
end

function handlers.onLocation(locationId) controller.location(locationId) end
...
function loader.config()
	...
	for id, path in pairs(paths.config.attribute) do
		...
		if attrData then attributeConfigs[id] = attrData end
	end
...
event.registerEventAsync("onInitDone", function() loader.init() end)
...
event.registerSaveHandler("TheArchadianAlchemist", function(filepath) return handlers.onSave() end)

event.registerLoadHandler("TheArchadianAlchemist",
	function(filepath, savedData) handlers.onLoad(filepath, savedData) end)
...
event.registerEventAsync("onMapJump", function(locationId) handlers.onLocation(locationId) end)
