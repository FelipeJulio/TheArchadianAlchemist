...
function controller.schedule(pollId)
	if pollId ~= controller.pollId then return end

	local interval = flow.interval(addresses.base, addresses)

	event.executeAfterMs(interval, function() controller.poll(pollId) end)
end

function controller.poll(pollId)
	if pollId ~= controller.pollId then return end

	if not controller.running or not controller.valid then return end

	if configData and configData.questline then unlock.update(addresses.base, addresses, configData.questline, symbols) end

	local flowEvent = flow.poll(addresses.base, addresses)
	if flowEvent then controller.dispatch(flowEvent, addresses.base, addresses, configData) end
...
function controller.dispatch(flowEvent, base, addrs, config)
	...
	if flowEvent == flowEvents.result then
		...
		if symbols then symbols.refresh() end

		return
	end
end

function controller.reset(base, addrs) flow.load(base, addrs) end
...
function controller.validateLocation(locationId) return locationId == controller.targetLocation end

function controller.setSymbols(symbolsModule) symbols = symbolsModule end

function controller.setConfig(config) configData = config end
