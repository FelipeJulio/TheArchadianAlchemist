...
function writer.upgrade(base, offset, subcategoryId, tierId, categoryId, config, flowStatus)
	if flowStatus == 3 and (not categoryId or categoryId == 0) then return false end

	local upgrades, gilValue = getUpgradeData(subcategoryId, tierId, categoryId, config, flowStatus)
	if not upgrades or not writer.validateUpgrades(upgrades) then return false end
...
function writer.validateUpgrades(upgrades)
	if not helpers.isTable(upgrades) or #upgrades == 0 then return false end

	for i = 1, math.min(#upgrades, 3) do
		local item = upgrades[i]
		if not helpers.isTable(item) or not helpers.getItemId(item) then return false end
	end
