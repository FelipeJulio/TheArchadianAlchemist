...
function equipment.get(id)
	if not equipment.validateId(id) then return nil end

	local idx = id - constants.equipmentBaseId
	return bpack.section13[idx]
end

function equipment.validateId(id) return id and id > 0 end

local function readElement(eq, elementId)
	if elementId < 1 or elementId > 8 then return nil end
...
local function readAttribute(eq, attributeId)
	if attributeId < 10 or attributeId > 26 then return nil end
...
function equipment.read(id, key, subid)
	...
	if key == "element" then return readElement(eq, subid) end
	if key == "attribute" then return readAttribute(eq, subid) end
...
function equipment.write(id, key, subid, value)
	...
	if key == "element" then
		if subid < 1 or subid > 8 then return false end
		return equipment.applyElement(eq, subid, value)
	elseif key == "attribute" then
		if subid < 10 or subid > 26 then return false end
...
function equipment.update(id, key, value)
	if not equipment.validateId(id) then return end
...
function equipment.clearAllElements(id)
	local eq = equipment.get(id)
	if not eq or not eq.elements then return end

	for elementId = 1, 8 do
		local name = elementNames[elementId]
		if name then eq.elements[name] = 0 end
	end
end

function equipment.clearState() equipment.state = {} end

function equipment.setState(state) equipment.state = state or {} end
