...
function element.get(base, addrs)
	...
	if equipmentId <= 0 then return 0 end

	local eq = equipment.get(equipmentId)
	if not eq or not eq.elements then return 0 end

	for id = 1, 8 do
		local name = elementNames[id]
		if name and eq.elements[name] == 1 then return id end
	end

	return 0
end

function element.write(base, addrs, elementId) mem.flags.set(base, addrs.current.currentElement, elementId or 0) end

local function clearElements(eq) for id = 1, 8 do equipment.applyElement(eq, id, 0) end end

function element.set(base, addrs, elementId)
	if elementId < 1 or elementId > 8 then return false end
